...
public class TestController {
    ...
    public WebResp<String> test(@RequestParam(value = "signal") int signal) {
        ...
        if (signal == 0) {
            ...
            throw new MyException(ErrorCode.RESOURCE_NOT_FOUND, ImmutableMap.of("请求异常", "请求资源不存在"));
        } else if (signal == 2) {
            ...
            try {
                ...
                throw new MyException(ErrorCode.REQUEST_VALIDATION_FAILED, ImmutableMap.of("请求异常", "<UNK>"), e.getCause());
            }
            return WebResp.success("测试成功", "获取数据成功");
        } else {
            throw new MyException(ErrorCode.REQUEST_VALIDATION_FAILED, ImmutableMap.of("请求异常", "参数不合法，signal只能为0、1、2"), new IllegalArgumentException("参数不合法，signal只能为0、1、2"));
        }
